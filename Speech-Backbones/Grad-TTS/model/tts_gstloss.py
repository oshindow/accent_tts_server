...
class GradTTSGST(BaseModule):
    ...
    @torch.no_grad()
    def forward(self, x, x_lengths, y, y_lengths_ref, n_timesteps, temperature=1.0, stoc=False, spk=None, acc=None, length_scale=1.0):
        """
        ...
        if self.gst:
            embedded_gst, pred_style = self.gst(y, y_lengths_ref)
    ...
    def compute_loss(self, x, x_lengths, y, y_lengths, spk=None, acc=None, out_size=None):
        ...
        if self.n_accents > 1:
            ...
            acc = None
        # Get encoder_outputs `mu_x` and log-scaled token durations `logw`
        mu_x, logw, x_mask = self.encoder(x, x_lengths, spk, acc) # mu_x(16, 80, 71) logw(16, 1 71) x_mask (16, 1, 71) 1 or 0
        ...
        y_mask = sequence_mask(y_lengths, y_max_length).unsqueeze(1).to(x_mask) # y_mask (16, 1, 196)
        attn_mask = x_mask.unsqueeze(-1) * y_mask.unsqueeze(2) # attn_mask (16,1,71,196)
        ...
        mu_y = torch.matmul(attn.squeeze(1).transpose(1, 2), mu_x.transpose(1, 2)) # attn (16,71,196)
        mu_y = mu_y.transpose(1, 2) # mu_y (16,80,196)

        # y_cut = y
        if self.gst:
            embedded_gst, pred_style = self.gst(y, y_lengths)
            embedded_gst = embedded_gst.repeat(1, mu_y.size(2), 1)
