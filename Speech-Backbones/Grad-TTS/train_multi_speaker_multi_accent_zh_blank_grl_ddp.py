...
log_dir = '/data2/xintong/gradtts/logs/new_exp_sg_acc_blank_grl_gst_ddp'
...
def run(rank, n_gpus):
    ...
    for epoch in range(1, n_epochs + 1):
        if rank == 0:
            ...
            with torch.no_grad():
                for item in test_batch:
                    x = item['x'].to(torch.long).unsqueeze(0).cuda(rank)
                    x_lengths = torch.LongTensor([x.shape[-1]]).cuda(rank)
                    y, y_lengths = item['y'].unsqueeze(0).cuda(rank), torch.LongTensor([item['y'].shape[1]]).cuda(rank)
