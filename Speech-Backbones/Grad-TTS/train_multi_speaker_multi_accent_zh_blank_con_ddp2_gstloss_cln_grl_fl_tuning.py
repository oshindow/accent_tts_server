...
import params
from model import GradTTSGST, GradTTSConformer, GradTTSConformerGST, GradTTSConformerGSTGRLFL
from data_sg_tuning import TextMelSpeakerDataset, TextMelSpeakerBatchCollate, TextMelSpeakerAccentDataset, TextMelSpeakerAccentBatchCollate
...
train_filelist_path = 'resources/filelists/zh_all/train_sg.txt'
...
log_dir = '/data2/xintong/gradtts/logs/new_exp_sg_acc_blank_conformer_gst_E15'
...
pretrained_model = 'logs/new_exp_sg_acc_blank_conformer_gst_E14/grad_255.pt'
...
def run(rank, n_gpus):
    ...
    model = GradTTSConformerGSTGRLFL(nsymbols, n_spks, spk_emb_dim, n_enc_channels,
                    filter_channels, filter_channels_dp, 
                    n_heads, n_enc_layers, enc_kernel, enc_dropout, window_size, 
                    n_feats, dec_dim, beta_min, beta_max, pe_scale, n_accents, grl=True, gst=True, cln=True).cuda(rank)
    ...
    for epoch in range(start_epoch + 1, n_epochs + 1):
        ...
        for i, batch in enumerate(loader):
                ...
                if model.module.grl:
                    dur_loss, prior_loss, diff_loss, spk_loss, gst_loss = model.module.compute_loss(x, x_lengths, # go gradtts compute loss
                                                                        y, y_lengths,
                                                                        spk=spk, acc=acc, out_size=out_size)
                    loss = sum([dur_loss, prior_loss, diff_loss, spk_loss, gst_loss])
                ...
                if rank == 0:
                    ...
                    if model.module.grl:
                        ...
                        msg = f'Epoch: {epoch}, iteration: {iteration} | dur_loss: {dur_loss.item()}, prior_loss: {prior_loss.item()}, diff_loss: {diff_loss.item()}, spk_loss: {spk_loss.item()}, gst_loss: {gst_loss.item()}, acc: {acc}, spk: {spk}, lr: {learning_rate}, {file}'
                    else:
                        msg = f'Epoch: {epoch}, iteration: {iteration} | dur_loss: {dur_loss.item()}, prior_loss: {prior_loss.item()}, diff_loss: {diff_loss.item()}, gst_loss: {gst_loss.item()} lr: {learning_rate}, {file}'
