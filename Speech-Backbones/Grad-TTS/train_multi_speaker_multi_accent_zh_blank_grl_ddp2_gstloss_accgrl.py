...
log_dir = '/data2/xintong/gradtts/logs/new_exp_sg_acc_blank_grl_gst_ddp2_lrfix_gstloss_1e4_accgrl'
...
def run(rank, n_gpus):
    ...
    model = GradTTSGST(nsymbols, n_spks, spk_emb_dim, n_enc_channels,
                    ...
                    n_feats, dec_dim, beta_min, beta_max, pe_scale, n_accents, grl=True, gst=True).cuda(rank)
    ...
    for epoch in range(start_epoch + 1, n_epochs + 1):
        ...
        for i, batch in enumerate(loader):
                ...
                if model.module.grl:
                    dur_loss, prior_loss, diff_loss, acc_loss, gst_loss = model.module.compute_loss(x, x_lengths, # go gradtts compute loss
                                                                        y, y_lengths,
                                                                        spk=spk, acc=acc, out_size=out_size)
                    loss = sum([dur_loss, prior_loss, diff_loss, acc_loss, gst_loss])
