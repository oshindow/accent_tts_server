...
def run(rank, n_gpus):
    ...
    for epoch in range(start_epoch + 1, n_epochs + 1):
        if rank == 0:
            ...
            with torch.no_grad():
                for i, item in enumerate(test_loader):
                    ...
                    spk = item['acc'].to(torch.long).cuda(rank)
                    acc = item['spk'].to(torch.long).cuda(rank)
        ...
        for i, batch in enumerate(loader):
                ...
                spk = batch['acc'].cuda(rank)
                acc = batch['spk'].cuda(rank)
                file = batch['filepath']
                ...
                if rank == 0:
                    ...
                    if model.module.grl:
                        ...
                        msg = f'Epoch: {epoch}, iteration: {iteration} | dur_loss: {dur_loss.item()}, prior_loss: {prior_loss.item()}, diff_loss: {diff_loss.item()}, acc_loss: {acc_loss.item()}, gst_loss: {gst_loss.item()}, acc: {acc}, spk: {spk}, lr: {learning_rate}, {file}'
                    else:
